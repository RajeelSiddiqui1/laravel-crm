            ...
            <div class="col-md-8 col-lg-6">
                <div class="card shadow rounded">
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ route('project_owner.tasks.update', $task->id) }}" id="taskForm">
                            @csrf
                            @method('PUT')
                            ...
                            <fieldset>
                                <div class="form-group">
                                    ...
                                    <div class="checkbox-group">
                                        @php
                                            $selectedManagers = is_string($task->managers) ? json_decode($task->managers, true) : ($task->managers ?? []);
                                        @endphp
                                        @foreach ($managers as $manager)
                                            <label>
                                                <input type="checkbox" name="managers[]" value="{{ $manager['id'] }}"
                                                    {{ in_array($manager['id'], old('managers', $selectedManagers)) ? 'checked' : '' }}>
                                                {{ $manager['name'] }}
                                            </label>
                                        @endforeach
                                    </div>
                            ...
                            <fieldset>
                                ...
                                <div class="form-group text-center mt-3">
                                    <label class="text-white d-block">Record New Audio (Admin)</label>
                            ...
                            <div class="form-group text-center mt-4">
                                ...
                                <a href="{{ route('project_owner.task') }}" class="btn btn-secondary ml-2">Cancel</a>
                            </div>
                        </form>
                    </div>
                </div>
