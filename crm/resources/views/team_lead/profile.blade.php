        ...
        <div class="col-lg-4 col-md-5">
            <div class="card profile-card shadow-sm border-0 h-100 animate__animated animate__fadeIn">
                <div class="card-body pt-4 text-center">
                    <img src="{{ $manager->image ? asset('images/team_leads/' . $manager->image) : 'https://avatar.iran.liara.run/public/28' }}"
                         alt="profile-image" class="profile rounded-circle border border-3 border-white shadow mb-3"
                         style="width: 120px; height: 120px; object-fit: cover;">
                    <h5 class="card-title mb-2 fw-bold">{{ $manager->name }}</h5>
                    <p class="card-text text-muted mb-3">{{ $manager->email }}</p>
                    @if(session('success'))
        ...
        <div class="col-lg-8 col-md-7">
            <div class="card shadow-sm border-0 animate__animated animate__fadeIn">
                ...
                <div class="card-body p-4">
                    <form method="POST" action="{{ route('team_lead.profile.update') }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')

                        <div class="mb-4">
                            <label for="name" class="form-label fw-semibold">Name</label>
                            <input type="text" name="name" id="name" value="{{ old('name', $manager->name) }}"
                        ...
                        <div class="mb-4">
                            ...
                            <input type="email" name="email" id="email" value="{{ old('email', $manager->email) }}"
